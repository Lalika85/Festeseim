var projects=[], shopItems=[], profile={}, savedQuotes=[], favorites=[];
        ...
        try {
            projects = JSON.parse(localStorage.getItem('fn133_projects')) || JSON.parse(localStorage.getItem('fn131_projects')) || [];
            shopItems = JSON.parse(localStorage.getItem('fn133_shop')) || JSON.parse(localStorage.getItem('fn131_shop')) || [];
            profile = JSON.parse(localStorage.getItem('fn133_profile')) || JSON.parse(localStorage.getItem('fn131_profile')) || {name:'',address:'',tax:'',phone:'',email:'',bank:'',logo:null};
            savedQuotes = JSON.parse(localStorage.getItem('fn133_quotes')) || [];
            favorites = JSON.parse(localStorage.getItem('fn133_favs')) || ["Ragasztó", "Festék", "Gipszkarton", "Csavar", "Kábel", "Profil"];
...
function bindEvents() {
    ...
    document.getElementById('dash-card-calc').addEventListener('click', () => window.switchTab('calc'));
        ...
        window.saveProject = function(){ 
            ...
            projects.unshift({id: Date.now(), client: n, phone: document.getElementById('inp-phone').value, email: document.getElementById('inp-email').value, address: document.getElementById('inp-address').value, note: document.getElementById('inp-note').value, start: document.getElementById('inp-start').value, end: document.getElementById('inp-end').value, status: status, rooms: [], docs: []}); 
            localStorage.setItem('fn133_projects', JSON.stringify(projects)); 
        ...
        window.renderCalendar = function(date) {
            ...
            else relevantProjects.forEach(p => { let col = getProjectColor(p.id); eventListEl.innerHTML += `<div class="list-item" onclick="window.openDetail(${p.id})"><div><span style="display:inline-block;width:10px;height:10px;background:${col};border-radius:50%;margin-right:5px;"></span><b>${p.client}</b><br><small>${p.start} - ${p.end}</small></div><i class="fas fa-chevron-right" style="color:#ccc;"></i></div>`; });
        };
        ...
        window.saveCurrentQuote = function(){ if(!quoteItems.length)return; const c=document.getElementById('quote-client-name').value||'Névtelen'; savedQuotes.unshift({id:Date.now(),client:c,date:document.getElementById('quote-date').value,total:quoteItems.reduce((a,b)=>a+b.totalGross,0),items:[...quoteItems]}); localStorage.setItem('fn133_quotes',JSON.stringify(savedQuotes)); window.renderSavedQuotes(); window.showToast('Mentve'); };
        ...
        window.deleteQuote = function(id){ if(!confirm('?'))return; savedQuotes=savedQuotes.filter(x=>x.id!=id); localStorage.setItem('fn133_quotes',JSON.stringify(savedQuotes)); window.renderSavedQuotes(); };
        ...
        window.saveProfile = function(){ profile.name=document.getElementById('prof-name').value; profile.address=document.getElementById('prof-address').value; profile.tax=document.getElementById('prof-tax').value; profile.phone=document.getElementById('prof-phone').value; profile.email=document.getElementById('prof-email').value; profile.bank=document.getElementById('prof-bank').value; localStorage.setItem('fn133_profile',JSON.stringify(profile)); window.showToast('Mentve'); window.closeModal(); window.updateProfilePreview(); };
        ...
        window.deleteDoc = function(idx) { if(!confirm("Törlöd?")) return; const p = projects.find(x => x.id == currentDetailId); p.docs.splice(idx, 1); localStorage.setItem('fn133_projects', JSON.stringify(projects)); window.renderDocsList(); };
        window.addRoom = function(){ const v=document.getElementById('new-room-input').value; const s=document.getElementById('new-room-size').value; if(!v)return; const p=projects.find(x=>x.id==currentDetailId); if(!p.rooms)p.rooms=[]; p.rooms.push(s?`${v} (${s}m²)`:v); localStorage.setItem('fn133_projects',JSON.stringify(projects)); document.getElementById('new-room-input').value=''; window.openDetail(currentDetailId); };
        window.deleteRoom = function(r){ if(!confirm('Törlöd?'))return; const p=projects.find(x=>x.id==currentDetailId); p.rooms=p.rooms.filter(x=>x!==r); localStorage.setItem('fn133_projects',JSON.stringify(projects)); window.openDetail(currentDetailId); };
        window.deleteCurrentProject = function(){ if(!confirm('Törlöd?'))return; projects=projects.filter(x=>x.id!=currentDetailId); localStorage.setItem('fn133_projects',JSON.stringify(projects)); window.closeModal(); window.renderDashboard(); window.switchTab('projects'); };
        ...
        window.saveItemEdit = function(){const id=document.getElementById('item-edit-id').value;const i=shopItems.find(x=>x.id==id);i.text=document.getElementById('item-edit-text').value;i.qty=document.getElementById('item-edit-qty').value;i.code=document.getElementById('item-edit-code').value;i.note=document.getElementById('item-edit-note').value;localStorage.setItem('fn133_shop',JSON.stringify(shopItems));window.closeModal();window.renderShop();if(currentDetailId)window.renderDetailList();}
        ...
        window.toggleShop = function(id) { const i=shopItems.find(x=>x.id==id); i.done=!i.done; localStorage.setItem('fn133_shop',JSON.stringify(shopItems)); window.renderShop(); if(document.getElementById('view-detail').style.display === 'flex') window.renderDetailList(); }
        window.delShop = function(id) { shopItems=shopItems.filter(x=>x.id!=id); localStorage.setItem('fn133_shop',JSON.stringify(shopItems)); window.renderShop(); if(document.getElementById('view-detail').style.display === 'flex') window.renderDetailList(); }
